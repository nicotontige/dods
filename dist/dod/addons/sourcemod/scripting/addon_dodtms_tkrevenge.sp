...
{
	TKRStatus = CreateConVar("dod_tms_tkrevenge", "1", "<1/0> = enable/disable TK Revenge Addon", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRUseForgiveMenu = CreateConVar("dod_tms_tkrenabletkmenu", "1", "<1/0> = enable/disable displaying a Menu with punishments on TK", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	ClientImmunity = CreateConVar("dod_tms_tkrimmunity", "1", "<1/0> = enable/disable Admins being immune from almost all actions", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRSaveTKs = CreateConVar("dod_tms_tkrsavetks", "1", "<1/0> = enable/disable saving player's TK count to file and load it on next connection", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRBanTime = CreateConVar("dod_tms_tkrbantime", "1440", "<#/0> = set time in minutes to ban team killers  -  0 = permanent", FCVAR_PLUGIN, true, 0.0)
	TKRMaxTKs = CreateConVar("dod_tms_tkrmaxtkcount", "6", "<#> = set max TK count for kicking/banning", FCVAR_PLUGIN, true, 1.0, true, 12.0)
	TKRTAEqTK = CreateConVar("dod_tms_tkrtasequaltk", "10", "<#> = set number of TAs that count as a TK", FCVAR_PLUGIN, true, 1.0, true, 20.0)
	TKRSpawnTKCount = CreateConVar("dod_tms_tkrspawntkcount", "3", "<#> = set number of TKs that are added to the count for spawn TKs", FCVAR_PLUGIN, true, 1.0, true, 5.0)
	TKRSpawnArea = CreateConVar("dod_tms_tkrspawnarea", "800", "<#> = set size of player's spawn area", FCVAR_PLUGIN, true, 400.0, true, 1000.0)
	TKRMeleeTKCount = CreateConVar("dod_tms_tkrmeleetkcount", "2", "<#> = set number of TKs that are added to the count for melee TKs", FCVAR_PLUGIN, true, 1.0, true, 5.0)
	TKRRampageTKCount = CreateConVar("dod_tms_tkrrampagetkcount", "3", "<0/#> = set number of TKs that are added to the count for continued TKing within X seconds  -  0 = disable rampage check", FCVAR_PLUGIN, true, 0.0, true, 5.0)
	TKRRampageTime = CreateConVar("dod_tms_tkrrampagetimespan", "10", "<#> = time in seconds that continuous TKing counts as Rampage", FCVAR_PLUGIN, true, 0.0, true, 15.0)
	TKRMaxPrevKick = CreateConVar("dod_tms_tkrkickstoban", "2", "<#> = set number of previous Kicks that result in a temp Ban", FCVAR_PLUGIN, true, 2.0, true, 5.0)
	TKRMaxPrevKickBT = CreateConVar("dod_tms_tkrkickstobantime", "10080", "<#> = time in minutes to ban Xtimes kicked TeamKillers", FCVAR_PLUGIN, true, 1.0)
	TKRMaxPrevBan = CreateConVar("dod_tms_tkrbanstopermban", "2", "<#> = set number of previous temp Bans that result in a permanent Ban", FCVAR_PLUGIN, true, 2.0, true, 10.0)
	TKRAction = CreateConVar("dod_tms_tkraction", "1", "<0/1> = action to take when max TK count is reached  -  0 = kick  -  1 = ban", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRSpawnTA = CreateConVar("dod_tms_tkrspawntaaction", "3", "<0/1/2/3> = action to take for SpawnTAs/TKs  -  0 = disabled  -  1 = slay attacker  -  2 = slay attacker & health for victim  -  3 = mirror damage  -  4 = only health for victim", FCVAR_PLUGIN, true, 0.0, true, 4.0)
	TKRMeleeTA = CreateConVar("dod_tms_tkrmeleetaaction", "3", "<0/1/2/3> = action to take for MeleeTAs/TKs  -  0 = disabled  -  1 = slay attacker  -  2 = slay attacker & health for victim  -  3 = mirror damage -  4 = only health for victim", FCVAR_PLUGIN, true, 0.0, true, 4.0)
	TKRGrenadeTKCount = CreateConVar("dod_tms_tkrcountnadetks", "1", "<1/0> = count grenade TKs as normal TKs  -  1 = raise TK count  -  0 = ignore grenade TKs", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRKarmaReduceTK = CreateConVar("dod_tms_tkrkarmareducetk", "10", "<#> = set number of KarmaPoints to reduce the TK count by one", FCVAR_PLUGIN, true, 1.0)
	TKRScoreKarma = CreateConVar("dod_tms_tkrscorekarma", "2", "<#> = set number of KarmaPoints to add for fulfilling objectives", FCVAR_PLUGIN, true, 1.0)
	TKRKillKarma = CreateConVar("dod_tms_tkrkillkarma", "1", "<#> = set number of KarmaPoints to add for killing enemies", FCVAR_PLUGIN, true, 1.0)
	TKRSlayFX = CreateConVar("dod_tms_tkrslayfx", "1", "<1/0> = enable/disable explosion sound/effects for slaying players", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRShowTKCount = CreateConVar("dod_tms_tkrshowtkcount", "1", "<1/0> = enable/disable showing team killer's TK count on increase/decrease", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRNoVictimDeath = CreateConVar("dod_tms_tkrnovictimdeath", "1", "<1/0> = enable/disable removing deaths caused by TKs from the scoreboard", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRAdminAutoSorry = CreateConVar("dod_tms_tkradminsorry", "1", "<1/0> = enable/disable making admins automatically saying 'Sorry' for TKs", FCVAR_PLUGIN, true, 0.0, true, 1.0)
	TKRForgiveMenuLayout = CreateConVar("dod_tms_tkrtkmenulayout", "abcd", "<options> = define TK Menu Layout:  a = Forgive! - b = DON'T Forgive! - c = Slap to 1hp! - d = Slay!", FCVAR_PLUGIN)
	TKRForgiveMenuTime = CreateConVar("dod_tms_tkrtkmenutime", "20", "<#/0> = time in seconds the menu stays open until an automatic choice is made  -  0 = stay open until manual choice", FCVAR_PLUGIN, true, 0.0, true, 60.0)
	TKRRespawnSlay = CreateConVar("dod_tms_tkrrespawnslay", "1", "<1/0> = enable/disable slaying team killer after respawn if 'Slay!' was chosen on dead player", FCVAR_PLUGIN, true, 0.0, true, 1.0)
